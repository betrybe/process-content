...
const getBuildChapterObj = async (markdownPath, yamlPath) => {
...
const groupFiles = (filesArr) => {
  const toot = filesArr.reduce(async (acc, curr, index, array) => {
    ...
    if (filesMatch) {
      ...
      const chapterContentAndIdObj = await getBuildChapterObj(markdownPath, yamlPath);

      return Promise.resolve([...acc, { ...chapterContentAndIdObj, path: markdownPath }]);
    }
...
//   console.log(chapterObjEntriesArray)

//   const toot = chapterObjEntriesArray.reduce(async (acc, curr) => {
...
//     const chapterObj = await getBuildChapterObj(curr)
...
//   //   getBuildChapterObj(type, commitId, rawContent, chapterObj);
//   // }
// };
...
const buildChapters = async (path) => {
  ...
  const chapterObj = await groupFiles(arrayOfFiles);
